...
static bool is_attack_critical(struct Damage* wd, block_list *src, block_list *target, int32 skill_id, int32 skill_lv, bool first_call)
{
	...
	if( sstatus->cri )
	{
		...
		if(wd->type == DMG_MULTI_HIT){	//Multiple Hit Attack Skills.
			...
			if(pc_checkskill(sd,MO_TRIPLEATTACK) && !skill_get_nk(MO_TRIPLEATTACK,NK_CRITICAL)) //Triple Attack
				return false;
		}
...
static int32 is_attack_piercing(struct Damage* wd, block_list *src, block_list *target, int32 skill_id, int32 skill_lv, int16 weapon_position)
{
	...
	if(src != nullptr) {
		...
		if( skill_id != PA_SACRIFICE && skill_id != CR_GRANDCROSS && skill_id != NPC_GRANDDARKNESS && skill_id != PA_SHIELDCHAIN && skill_id != KO_HAPPOKUNAI && skill_id != AM_ACIDTERROR && skill_id != AM_DEMONSTRATION)
...
static void battle_calc_multi_attack(struct Damage* wd, block_list *src,block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	switch (skill_id) {
		case MO_TRIPLEATTACK:
			wd->div_ = skill_get_num(MO_TRIPLEATTACK,skill_lv);
			wd->type = DMG_MULTI_HIT;
			break;
		case AS_SONICBLOW:
			wd->type = DMG_MULTI_HIT;
...
static int32 battle_calc_attack_skill_ratio(struct Damage* wd, block_list *src,block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
...
#else
			skillratio += 40 * skill_lv +  sstatus->agi;
#endif
			break;
		case RG_INTIMIDATE:
		...
		case AM_ACIDTERROR:
		{
			...
			if(sd)
			{
				if (pc_checkskill(sd, AM_LEARNINGPOTION))
					skillratio += 20 * skill_lv;
				...
				if(bottleIndex >= 0)
				{
					...
					skillratio += (tstatus->vit * tstatus->vit) / (25 / skill_lv);
				}
...
static struct Damage battle_calc_weapon_attack(block_list *src, block_list *target, uint16 skill_id, uint16 skill_lv, int32 wflag)
{
	...
	if ((!skill_id || skill_id == PA_SACRIFICE || skill_id == NPC_EARTHQUAKE)  && tstatus->flee2 && rnd()%1000 < tstatus->flee2) {
	...
	if (is_attack_critical(&wd, src, target, skill_id, skill_lv, false) && skill_id && skill_id != MO_TRIPLEATTACK) {
		if (sd) { //Check for player so we don't crash out, monsters don't have bonus crit rates [helvetica]
			...
			if (is_attack_left_handed(src, skill_id))
				wd.damage2 = (int64)floor((float)((wd.damage2 * (1.5f))));
		} else
			wd.damage = (int64)floor((float)(wd.damage * 1.5f));
			...
			default: {
				...
				switch(skill_id) {
					...
					case WZ_FIREPILLAR:
						...
						skillratio += -60 + 40 * skill_lv; //20% MATK each hit
						break;
					case WZ_SIGHTRASHER:
						skillratio += 20 * skill_lv;
						break;
...
// rAthena
	...
	{ "feature.dynamicnpc_timeout",         &battle_config.feature_dynamicnpc_timeout,      1000,   20000,  INT_MAX,        },
